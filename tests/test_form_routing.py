...
def _route(gr, assets=None, priors=(50_000.0, 50_000.0)):
    """Route with enough history to avoid the single-year review flag, so
    threshold behaviour can be tested in isolation."""
    return route_form_variant(gr, assets, prior_year_gross_receipts=priors)


# --- the three variants ---------------------------------------------------

def test_small_organisation_files_990n():
    r = route_form_variant(30_000.0, 10_000.0, prior_year_gross_receipts=(29_000.0, 31_000.0))
    assert r.form == FORM_990N


def test_mid_sized_organisation_files_990ez():
    r = route_form_variant(120_000.0, 200_000.0, prior_year_gross_receipts=(118_000.0, 122_000.0))
    assert r.form == FORM_990EZ


def test_large_organisation_files_full_990():
    r = route_form_variant(400_000.0, 900_000.0, prior_year_gross_receipts=(390_000.0, 410_000.0))
...
def test_high_assets_force_the_full_990_despite_low_receipts():
    ...
    r = route_form_variant(60_000.0, 800_000.0, prior_year_gross_receipts=(60_000.0, 60_000.0))
    assert r.form == FORM_990
...
def test_ez_requires_both_conditions():
    below_both = route_form_variant(
        150_000.0, 400_000.0, prior_year_gross_receipts=(150_000.0, 150_000.0)
    )
    assets_over = route_form_variant(
        150_000.0, 600_000.0, prior_year_gross_receipts=(150_000.0, 150_000.0)
    )
...
def test_unknown_assets_choose_the_full_return_and_ask_for_review():
    ...
    r = route_form_variant(150_000.0, None, prior_year_gross_receipts=(150_000.0, 150_000.0))
    assert r.form == FORM_990
...
def test_threshold_edges(gross_receipts, expected):
    r = route_form_variant(
        gross_receipts,
        1_000.0,
        prior_year_gross_receipts=(gross_receipts, gross_receipts),
    )
...
def test_asset_limit_edge():
    at_limit = route_form_variant(
        ...
        prior_year_gross_receipts=(100_000.0, 100_000.0),
    )
...
def test_a_single_spike_does_not_move_a_small_organisation():
    ...
    r = route_form_variant(90_000.0, 5_000.0, prior_year_gross_receipts=(20_000.0, 20_000.0))
    assert r.form == FORM_990N
    assert r.basis == "rolling_average_3yr"


def test_one_year_of_data_is_flagged_for_review():
    r = route_form_variant(30_000.0, 5_000.0)
    assert r.form == FORM_990N
    assert r.requires_review
...
def test_sandbox_company_routes_to_990n():
    ...
    r = route_form_variant(10_605.77, 23_436.29)
    assert r.form == FORM_990N
    assert r.requires_review
    assert any("single period" in x for x in r.review_reasons)
